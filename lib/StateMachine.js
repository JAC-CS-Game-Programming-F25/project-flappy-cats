...
export default class StateMachine {
	...
	change(stateName, enterParameters) {
		...
		// Save the current state name for persistence
		// Don't save during restore process to prevent infinite loops
		if (this.gameController && stateName !== 'restore' && !this.isRestoring) {
			this.gameController.saveCurrentState(stateName);
		}
	}
